...
% rev2
\definechangesauthor[color=purple]{r2}
\setauthormarkuptext{}
...
\title{\replaced{Noninvasive prenatal exome sequencing diagnostic utility limited by sequencing depth and fetal fraction}{Noninvasive prenatal exome sequencing inefficient for detecting single-gene disorders -- problems and possible solutions}}
...
\begin{itemize}
    ...
    \item Fetal exome sequencing (ES) provides \added{10-}20\% diagnostic yield for structural abnormalities after normal karyotype \& microarray
\end{itemize}
...
During pregnancy, placental DNA \added{(cell free fetal DNA, cffDNA)} is released into maternal circulation, enabling noninvasive interrogation of fetal genetics\deleted{(noninvasive prenatal testing, NIPT)}.
\replaced{cffDNA has}{NIPT has a} well-established clinical utility in screening for common chromosomal abnormalities such as Down syndrome with high sensitivity and specificity\cite{mackie:2017aa}.
...
Scotchman et al. provide a \replaced{more detailed discussion about the development of noninvasive fetal genetic testing, and we refer intereseted readers to their work}{n excellent review summarizing the history of noninvasive testing} \cite{scotchman:2020aa}.
...
Such an argument, however, dismisses (1) the ethical and practical issues surrounding the necessity of involving the biological father, (2) the fact that many genetic disorders arise due to \textit{de novo} mutations, \deleted{and} (3) the understandable fear and apprehension around invasive testing (especially for rare conditions)\added{, and (4) the cost and limited availability of invasive procedures}.
...
\added{In research settings, three groups have performed/attempted fetal genome sequencing from cell-free DNA \cite{lo:2010aa,kitzman:2012aa,fan:2012aa}.}
\added{In a review of noninvasive fetal sequencing work,} Snyder et al. \replaced{illustrate}{provide a review of previous attempts to perform noninvasive fetal genome sequencing, illustrating} the cost-infeasibility and \replaced{suggest}{suggesting} more targeted approaches such as exome sequencing (ES) \cite{snyder:2013aa}.
As an exploratory exercise, we performed ES on cell-free DNA (cfES) from three pregnant women.
\added{All three women had singleton pregnancies with suspected genetic conditions identified by a genetic counselor.
...
\added{Either due to family history of genetic condition or because of identified fetal abnormalities in the current pregnancy, the patients all pursued genetic workup through the genetic counselors separate from their participation in this study.
...
\replaced{Because existing germline variant calling pipelines would not account for the combination of read fractions from maternal and fetal DNA, w}{W}e processed the data using a novel analytic pipeline developed in Snakemake \cite{koster:2012aa} using Anaconda environments for reproducibility (provided in supplemental materials).
...
We found, as others have, a higher proportion of fetal reads falling below 150 basepairs; \replaced{however, we also note the increased observed dispersion of fetal read length led to a higher proportion of fetal reads longer than 200 basepairs (\cref{fig:c3fragLen}).}{however, we also observed a slightly higher proportion of longer reads, as well.}
\added{Limited to the single case, we cannot comment on the significance or generalizability of the increased dispersion.}
...
\added{
To move NBS into the \replaced[id=r2]{fetal}{neonatal} period we must develop noninvasive methodologies agnostic to paternity.
\added[id=r2]{Having prenatal genetic diagnoses will allow for optimized care in the immediate newborn period and the further development of \emph{in utero} therapies.}
...
We must also recognize the ethical issues with requiring the biological father in neonatal genetic screening programs\replaced[id=r2]{; many family structures do not include the biological father and requiring both parents to miss work for multiple appointments may exclude low-income parents.}{, including the financial burden of the father missing work for additional appointments.}
